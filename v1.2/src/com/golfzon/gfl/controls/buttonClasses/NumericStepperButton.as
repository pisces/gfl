...
public class NumericStepperButton extends SimpleButton
{
	...
	override protected function getSkinDefinition():Class {
		if( _direction == NumericStepperButton.NEXT ) {
			if( phase == PHASE_UP )
				return replaceNullorUndefined(getStyle(StyleProp.NEXT_ARROW_UP_SKIN), NEXT_ARROW_UP_SKIN);
			if( phase == PHASE_OVER )
				return replaceNullorUndefined(getStyle(StyleProp.NEXT_ARROW_OVER_SKIN), NEXT_ARROW_OVER_SKIN);
			if( phase == PHASE_DOWN )
				return replaceNullorUndefined(getStyle(StyleProp.NEXT_ARROW_DOWN_SKIN), NEXT_ARROW_DOWN_SKIN);
			if( phase == PHASE_SELECTED )
				return replaceNullorUndefined(getStyle(StyleProp.NEXT_ARROW_DOWN_SKIN), NEXT_ARROW_DOWN_SKIN);
			return replaceNullorUndefined(getStyle(StyleProp.NEXT_ARROW_DISABLED_SKIN), NEXT_ARROW_DISABLED_SKIN);
		}
		
		if( phase == PHASE_UP )
			return replaceNullorUndefined(getStyle(StyleProp.PREV_ARROW_UP_SKIN), PREV_ARROW_UP_SKIN);
		if( phase == PHASE_OVER )
			return replaceNullorUndefined(getStyle(StyleProp.PREV_ARROW_OVER_SKIN), PREV_ARROW_OVER_SKIN);
		if( phase == PHASE_DOWN )
			return replaceNullorUndefined(getStyle(StyleProp.PREV_ARROW_DOWN_SKIN), PREV_ARROW_DOWN_SKIN);
		if( phase == PHASE_SELECTED )
			return replaceNullorUndefined(getStyle(StyleProp.PREV_ARROW_DOWN_SKIN), PREV_ARROW_DOWN_SKIN);
		return replaceNullorUndefined(getStyle(StyleProp.PREV_ARROW_DISABLED_SKIN), PREV_ARROW_DISABLED_SKIN);
	}
