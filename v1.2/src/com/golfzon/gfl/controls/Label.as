...
public class Label extends ComponentBase implements IUITextFieldClient
{
	...
	override protected function createChildren():void {
		...
		textField.multiline = textField.editable = false;
		textField.selectable = _selectable;
		textField.text = text;
		...
		setLabelTextFormat();
		addChild(maskShape);
		textField.addEventListener(TextEvent.TEXT_FORMAT_CHANGED, textFormatChangedHandler, false, 0, true);
		addChild(textField);
		setViewMetrics();
	}
	...
	private function setLabelTextFormat():void {
		...
		format.align = align;
		format.color = rollOvered ?
			replaceNullorUndefined(getStyle(StyleProp.TEXT_ROLL_OVER_COLOR), 0x000000) :
