...
public class ScrollBarBase extends ComponentBase implements Null
{
	...
	gz_internal var DOWN_ARROW_DISABLED_SKIN:Class;
	gz_internal var DOWN_ARROW_DOWN_SKIN:Class;
	gz_internal var DOWN_ARROW_OVER_SKIN:Class;
	gz_internal var DOWN_ARROW_UP_SKIN:Class;
	gz_internal var THUMB_DOWN_SKIN:Class;
	gz_internal var THUMB_OVER_SKIN:Class;
	gz_internal var THUMB_UP_SKIN:Class;
	gz_internal var TRACK_DISABLED_SKIN:Class;
	gz_internal var TRACK_SKIN:Class;
	gz_internal var UP_ARROW_DISABLED_SKIN:Class;
	gz_internal var UP_ARROW_DOWN_SKIN:Class;
	gz_internal var UP_ARROW_OVER_SKIN:Class;
	gz_internal var UP_ARROW_UP_SKIN:Class;
	...
	override protected function commitProperties():void {
		...
		if( localScrollPositionChanged ) {
			...
			_scrollPosition = localPositionToTargetPosition(_localScrollPosition);
			scroll();
			dispatchScroll();
		}
		...
		if( scrollPositionChanged ) { 
			...
			_localScrollPosition = targetPositionToLocalPosition(_scrollPosition);
			scroll();
		}
	}
	...
	override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void {
		...
		downArrow.width = upArrow.width = isHorizontal() ? arrowSize : unscaledWidth;
		downArrow.height = upArrow.height = isHorizontal() ? unscaledHeight : arrowSize;
		
		downArrow.y = isHorizontal() ? 0 : unscaledHeight - arrowSize;
		upArrow.x = isHorizontal() ? unscaledWidth - arrowSize : 0;
		upArrow.y = 0;
	...
	gz_internal function moveThumb():void {
		if( isHorizontal() )
			thumb.x = positionToCoordinate(_localScrollPosition);
		else
			thumb.y = positionToCoordinate(_localScrollPosition);
	}
	...
	gz_internal function setThumbProperties():void {
		thumb.visible = downArrow.enabled = track.enabled = upArrow.enabled = _maxScrollPosition > 0;
			
		var maxThumbSize:Number = getSizeByDirection() - arrowSize * 2;
		var ratio:Number = pageSize / viewSize;
		var newThumbSize:Number = Math.max(_minThumbSize, maxThumbSize / ratio);
		localMaxScrollPosition = maxThumbSize - newThumbSize;
		
		thumb.width = isHorizontal() ? newThumbSize : unscaledWidth;
		thumb.height = isHorizontal() ? unscaledHeight : newThumbSize;
	}
	...
	public function lineScroll(line:Number):void {
		var value:Number = isHorizontal() ? _scrollPosition + line * -1 : _scrollPosition + line;
		var newScrollPosition:Number = getCurrentScrollPosition(Math.round(value));
	...
	public function setScrollProperties(pageSize:Number, viewSize:Number):void {
		...
		_maxScrollPosition = Math.ceil(Math.max(0, pageSize - viewSize) / _lineScrollSize);
		setThumbProperties();
		moveThumb();
	}
	...
	private function dispatchScroll(delta:Number=0):void {
		dispatchEvent(new ScrollEvent(ScrollEvent.SCROLL, false, false, null, _scrollPosition, _direction, delta));
	}
	...
	private function getCurrentScrollPosition(value:Number):Number {
		return Math.min(Math.max(0, value), _maxScrollPosition);
	}
	...
	private function isHorizontal():Boolean {
		return _direction == ScrollBarDirection.HORIZONTAL;
	}
	...
	private function localPositionToTargetPosition(value:Number):Number {
		return Math.round(value * _maxScrollPosition / localMaxScrollPosition);
	}
	...
	private function targetPositionToLocalPosition(value:Number):Number {
		return value * localMaxScrollPosition / _maxScrollPosition;
	}
