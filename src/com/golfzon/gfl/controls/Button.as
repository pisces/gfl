...
public class Button extends SimpleButton
{
	...
	private var selectedDownSkin:Class;
	
	[Embed(source="/assets/GZSkin.swf", symbol="Button_disabledSkin")]
	private var selectedDisabledSkin:Class;
	...
	override protected function getSkinDefinition():Class {
		...
		if( phase == PHASE_UP )
			return replaceNullorUndefined(getStyle(StyleProp.SELECTED_UP_SKIN), selectedDownSkin);
		if( phase == PHASE_OVER )
			return replaceNullorUndefined(getStyle(StyleProp.SELECTED_OVER_SKIN), selectedDownSkin);
		if( phase == PHASE_DOWN )
			return replaceNullorUndefined(getStyle(StyleProp.SELECTED_DOWN_SKIN), selectedDownSkin);
		return replaceNullorUndefined(getStyle(StyleProp.SELECTED_DISABLED_SKIN), selectedDisabledSkin);
	}
	...
	override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void {
		...
		if( currentIcon ) {
			currentIcon.x = _label ? labelInstance.x - currentIcon.width : Math.floor((unscaledWidth - currentIcon.width) / 2);
	...
	private function createIcon():void {
		...
		if( definition ) {
			...
			if( currentIcon is InteractiveObject )
				InteractiveObject(currentIcon).mouseEnabled = false;
				
			addChild(currentIcon);
		}
	}
	
	private function createLabelInstance():void {
		if( !labelInstance ) {
	...
	private function getTextColor():uint {
		if( toggle && selected )
			return replaceNullorUndefined(getStyle(StyleProp.TEXT_SELECTED_COLOR), 0x7A7A7A);
		if( phase == PHASE_OVER )
			return replaceNullorUndefined(getStyle(StyleProp.TEXT_ROLL_OVER_COLOR), 0x7A7A7A);
		if( phase == PHASE_DOWN )
			return replaceNullorUndefined(getStyle(StyleProp.TEXT_SELECTED_COLOR), 0x7A7A7A);
		return replaceNullorUndefined(getStyle(StyleProp.COLOR), 0x7A7A7A);
	}
	...
	private function getTextWidth():Number {
		...
		return _label ? pl + pr + labelInstance.measuredWidth : Math.max(cw, minWidth);
	}
	...
	private function getTextHeight():Number {
		...
		return _label ? pt + pb + labelInstance.measuredHeight : Math.max(ch, minHeight);
	}
	...
	private function removeLabelInstance():void {
		if( labelInstance && contains(labelInstance) ) {
			removeChild(labelInstance);
			labelInstance = null;
		}
	}
