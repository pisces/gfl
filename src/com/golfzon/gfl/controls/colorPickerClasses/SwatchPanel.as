...
public class SwatchPanel extends ComponentBase implements IColorSelectableObject
{
	...
	override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void {
    	...
    	selectionBox.width = getSumBorderColunmWidth();
		selectionBox.height = getSumBorderRowHeight();
		
		selectionBox.graphics.clear();
		selectionBox.graphics.lineStyle(1, 0x0, 1);
		selectionBox.graphics.drawRect(-borderThickness, -borderThickness, selectionBox.width, selectionBox.height);
	}
	...
	private function colorListGoOutChcek(x:int, y:int):Boolean {
		return x > 0 && x < getSumBorderColunmWidth() * columnCount &&
				y > 0 && y < getSumBorderRowHeight() * rowCount;
	}
	...
	private function configureListeners():void {
		colorList.addEventListener(MouseEvent.MOUSE_MOVE, moveHandler);
		colorList.addEventListener(MouseEvent.MOUSE_DOWN, mouseDownHandler);
		
		selectionBox.addEventListener(MouseEvent.MOUSE_DOWN, mouseDownHandler);
	}
	...
	private function selectionMove(valueX:int, valueY:int):void {
		selectionBox.x = valueX + colorList.x ;
		selectionBox.y = valueY + colorList.y ;
	}
	...
	private function moveSelectionBoxFromColor():void {
		for ( var i:Number = 0; i < dataProvider.length; i++ ) {
			if( selectedColor == dataProvider[i] ) {
				...
				selectionMove( x * getSumBorderColunmWidth() + borderThickness, y * getSumBorderRowHeight() + borderThickness);
			}
	...
	private function mouseDownHandler(event:MouseEvent):void {
		var color:uint = dataProvider[getArrayAddress(selectionBox.x, selectionBox.y)];
		selectedColor = color;		
