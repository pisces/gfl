 ...
 *	@Modify
 *	@Description
 * 	@includeExample		ButtonBarSample.as
 */	
public class ButtonBar extends ButtonBarBase
{
	//--------------------------------------------------------------------------
	//
	//	Instance variables
    ...
    override protected function getWidth():Number {
		...
		for( var i:uint = 0; i<columnSize; i++ ) {
			if( i != columnSize - 1 )	width += buttonList[i].width + replaceNullorUndefined(getStyle(StyleProp.HORIZONTAL_GAP), 0);
    ...
    override protected function getHeight():Number {
		...
		for( var i:uint = 0; i<rowSize; i++ ) {
			if( i != rowSize - 1 )	height += buttonList[i].height + replaceNullorUndefined(getStyle(StyleProp.VERTICAL_GAP), 0);
	...
	override protected function setButtonsGap():void {
		var hGap:Number = replaceNullorUndefined(getStyle(StyleProp.HORIZONTAL_GAP), 0);
		var vGap:Number = replaceNullorUndefined(getStyle(StyleProp.VERTICAL_GAP), 0);
