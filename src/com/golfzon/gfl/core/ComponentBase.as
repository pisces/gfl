...
public class ComponentBase extends Sprite
	implements IComponent, IInteractiveObject, IStyleClient
{
    //--------------------------------------------------------------------------
	//
	//	Class variables
	 ...
	 *	스테이지에 추가됐는지 여부
	 */
	protected var addedToStage:Boolean;
     ...
     */
    public function setActureSize(w:Number, h:Number):void {
	...
	private function addReservedChildList():void {
		var index:int = 0;
		for each( var obj:Object in reservedChildList ) {
			if( obj.index < 0 )	addChildAt(obj.child, index++);
			else				addChildAt(obj.child, obj.index);
			reservedChildList = [];
		}
	}
    ...
    private function createToolTip():void {
    	if( !tooltipInstance ) {
    		tooltipInstance = ToolTip.create(tooltip);
    		stage.addEventListener(MouseEvent.MOUSE_MOVE, stage_mouseMoveHandler, false, 0, true);
    		systemManager.stage.addChild(tooltipInstance);
    	}
    }
    ...
    private function removeToolTip():void {
    	if( tooltipInstance ) {
    		...
    		systemManager.stage.removeChild(tooltipInstance);
	 ...
	 */
    protected function resetOrigineSize():void {
	 ...
	 */
    protected function setEnabledState():void {
    ...
    private function setToolTip():void {
    	if( tooltip ) {
			...
			TweenMax.to(tooltipInstance, duration, {alpha:1});
    	} else {
    		removeToolTip();
    	}
    }
    ...
    private function setUnscaledWidth():void {
    	origineWidth = _unscaledWidth;
		_unscaledWidth = isNaN(_minWidth) ? _measureWidth : Math.max(_minWidth, _measureWidth);
    }
    ...
    private function setUnscaledHeight():void {
    	origineHeight = _unscaledHeight;
    	_unscaledHeight = isNaN(_minHeight) ? _measureHeight : Math.max(_minHeight, _measureHeight);
    }
	...
	public function get systemManager():SystemManager {
		...
		var p:DisplayObject = parent;
		while( p ) {
			if( p is IComponent && IComponent(p).systemManager )
				_systemManager = IComponent(p).systemManager;
