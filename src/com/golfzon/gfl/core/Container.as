...
public class Container extends ScrollControlBase implements IListItemRenderer
{
	...
	override public function removeChild(child:DisplayObject):DisplayObject {
		...
		_child.removeEventListener("widthChanged", child_resizeHandler);
		_child.removeEventListener("heightChanged", child_resizeHandler);
	...
	private function setContentPaneChild(target:DisplayObject):DisplayObject {
		...
		invalidateDisplayList();
		target.addEventListener("widthChanged", child_resizeHandler, false, 0, true);
		target.addEventListener("heightChanged", child_resizeHandler, false, 0, true);
		return target;
	}
	 ...
	 *	@private
	 */
	private function updateChildDisplay():void {
		if( invalidSize() )
			return;
